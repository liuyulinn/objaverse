...
for item in model_paths:
    ...
    command = f'blenderproc run {script_file} --object-path {path} --output_dir {group, args.output_dir} --num-views {args.num_views} --resolution {args.resolution} --radius {args.radius} --scale {args.scale} --random {args.random}'
    cmds.append(command)
    #cmds.append(item)
    uids.append(uid)
